...
"""
Модуль с GRU моделью для прогнозирования временных рядов.
"""
...
class GRUModel:
    """
    Класс для создания, обучения и использования GRU модели.
    """
    
    def __init__(self, input_shape: Tuple[int, int], output_size: int,
                output_activation: str = 'linear', gru_units: List[int] = [50, 50],
        ...
        logger.info(f"Инициализирована GRU модель с параметрами: "
                   f"input_shape={input_shape}, output_size={output_size}, "
                   f"gru_units={gru_units}, dropout_rate={dropout_rate}")
    ...
    def build_model(self) -> None:
        ...
        # Первый слой GRU и регуляризация
        model.add(GRU(self.gru_units[0], return_sequences=len(self.gru_units) > 1,
                     input_shape=self.input_shape, activation='tanh',
                     recurrent_activation='sigmoid'))
        model.add(BatchNormalization())
        model.add(Dropout(self.dropout_rate))
        
        # Дополнительные слои GRU (если указаны)
        for i in range(1, len(self.gru_units)):
            return_sequences = i < len(self.gru_units) - 1
            model.add(GRU(self.gru_units[i], return_sequences=return_sequences))
            model.add(BatchNormalization())
            model.add(Dropout(self.dropout_rate))
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, 
             ...
             model_path: str = 'models/gru_model.h5',
